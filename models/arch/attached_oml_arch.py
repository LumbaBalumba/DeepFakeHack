...
class Attached_Net_Arch(nn.Module):
    def __init__(self, model_type = 'resnet', weights = None,  freeze_source = 0, custom_pretrained = None, extra_layers=None):
        super().__init__()
    ...
    def parse_layers(self, layers):
        ...
        for layer in layers:
            ...
            layer_list.append((f"custom-layer{iteration}-{layer_name}", getattr(nn, layer_name)(**layer_params)))
            iteration += 1
